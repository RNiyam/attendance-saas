...
type LeavePolicyTemplate = {
  ...
  startDate: string;
  endDate: string;
  leaves?: { id: number; leaveName: string; leaveCode: string; annualQuota: string }[];
};
...
export default function LeavePolicySetupPage() {
  const router = useRouter();
  const [setupFlags, setSetupFlags] = useState(() => readSetupFlags());
  const [templates, setTemplates] = useState<LeavePolicyTemplate[]>([]);
  const [loading, setLoading] = useState(true);
  const [openMenuId, setOpenMenuId] = useState<number | null>(null);

  const load = useCallback(async () => {
    const res = await authenticatedFetch(`${apiBaseUrl}/api/leave/policy-templates`);
    if (res.ok) {
      const data = (await res.json()) as LeavePolicyTemplate[];
  ...
  useEffect(() => {
    ...
    const onDoc = (e: MouseEvent) => {
      ...
      if (target?.closest?.("[data-leave-menu]")) return;
      setOpenMenuId(null);
    };
  ...
  const handleMenuAction = async (action: (typeof menuItems)[number], template: LeavePolicyTemplate) => {
    setOpenMenuId(null);

    if (action === "Edit") {
      router.push(`/dashboard/setup/leave-policy/${template.id}`);
      return;
    }

    if (action === "Manage Rules") {
      router.push(`/dashboard/setup/leave-policy/${template.id}?tab=assign`);
      return;
    }

    if (action === "Clone") {
      const res = await authenticatedFetch(`${apiBaseUrl}/api/leave/policy-templates/${template.id}/clone`, {
        method: "POST",
      });
      if (res.ok) {
        await load();
    ...
    if (action === "Delete") {
      ...
      const res = await authenticatedFetch(`${apiBaseUrl}/api/leave/policy-templates/${template.id}`, {
        method: "DELETE",
      });
      if (res.ok) {
        await load();
        if (templates.length <= 1) {
          writeSetupFlag("leavePolicyDone", false);
          setSetupFlags(readSetupFlags());
        }
      }
    }
        ...
        <section className="mt-5 min-h-[calc(100vh-210px)] rounded-xl border border-[#D6DEE9] bg-white shadow-md shadow-black/[0.05]">
          <div className="px-6 py-7">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <h2 className="text-[16px] font-black text-[#0F0F1A]">Leave Policy</h2>
                <p className="mt-1 text-[14px] font-medium text-[#667085]">
                  Create leave types and quotas to manage employee time off
                </p>
              </div>
              <button
                type="button"
                onClick={() => router.push("/dashboard/setup/leave-policy/add")}
            ...
            {loading ? (
              <p className="mt-8 text-[13px] font-medium text-[#98A2B3]">Loading leave policies...</p>
            ) : templates.length === 0 ? (
              <div className="mt-8 max-w-[980px] rounded-lg border border-[#E5EAF2] bg-white px-6 py-10 text-center">
                <p className="text-[14px] font-medium text-[#667085]">No leave policies added</p>
              </div>
            ) : (
                  ...
                  >
                    <div className="min-w-0 pr-2">
                      ...
                      <p className="mt-2 text-[12px] font-medium text-[#667085]">
                        Leaves Added: {template.leaves?.length ?? 0}
        ...
        <div className="ml-auto flex max-w-[360px] gap-3">
          ...
          <button
            ...
            onClick={() => {
              writeSetupFlag("leavePolicyDone", true);
              setSetupFlags(readSetupFlags());
              router.push(setupHubHref("leavePolicyDone"));
            }}
